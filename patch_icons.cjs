...
code = code.replace(
  /icon={LayoutGrid}\s*label={t\.dailyPanchanga}/,
  "icon={Sun}\n            label={t.dailyPanchanga}",
);
code = code.replace(
  /icon={Calendar}\s*label={lang === "hi" \? "दैनिक समय-सारणी" : "Today Schedule"}/,
  'icon={ListTodo}\n            label={lang === "hi" ? "दैनिक समय-सारणी" : "Today Schedule"}',
);
code = code.replace(
  /icon={Clock}\s*label={lang === "hi" \? "वैदिक होरा" : "Vedic Horas"}/,
  'icon={Hourglass}\n            label={lang === "hi" ? "वैदिक होरा" : "Vedic Horas"}',
);
code = code.replace(
  /icon={Calendar}\s*label={t\.monthCalendar}/,
  "icon={CalendarDays}\n            label={t.monthCalendar}",
);
code = code.replace(
  /icon={Sparkles}\s*label={lang === "hi" \? "पर्व व व्रत" : "Festivals & Vratas"}/,
