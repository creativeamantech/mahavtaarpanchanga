...
function replaceInterval(name, startVar, endVar) {
  code = code.replace(
    new RegExp(`${name}: \\{\\s*start: formatTimeHMS\\(${startVar}\\),\\s*end: formatTimeHMS\\(${endVar}\\)`),
  ...
  code = code.replace(
    new RegExp(`${name}: \\{\\s*start: formatTimeHMS\\(${startVar}\\),\\s*end: formatTimeHMS\\(${endVar}\\),\\s*name:`),
