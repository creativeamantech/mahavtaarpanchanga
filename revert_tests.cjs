...
const targetStr = `    expect(TITHI_ELEMENT_MAPPING[1]).toBe("tejas");
...
const repStr = `    expect(TITHI_ELEMENT_MAPPING[1]).toBe("prithvi");
    expect(TITHI_ELEMENT_MAPPING[2]).toBe("prithvi");
...
const t2 = `    expect(periods[0].startElement).toBe("akash");
    expect(periods[0].endElement).toBe("akash");`;
const r2 = `    expect(periods[0].startElement).toBe("prithvi");
    expect(periods[0].endElement).toBe("prithvi");`;

content = content.replace(t2, r2);

fs.writeFileSync(file, content);
console.log('reverted tests');
