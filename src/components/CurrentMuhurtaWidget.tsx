...
export const CurrentMuhurtaWidget: React.FC<CurrentMuhurtaWidgetProps> = ({ data, lang }) => {
  ...
  const {
    ...
    timeFormatted: currentTimeString,
    isToday,
  } = getCityCurrentMinutes();
  ...
  const isIntervalActive = (
    interval?: TimingInterval | null,
  ): { active: boolean; remainingMins: number; progress: number } => {
    ...
    if (endM > startM) {
      ...
      if (currentMins >= startM || currentMins <= endM) {
        ...
        const elapsed =
          currentMins >= startM ? currentMins - startM : 24 * 60 - startM + currentMins;
  ...
  return (
    <div id="current-muhurta-widget" className="glass-card rounded-[1.5rem] p-5 sm:p-6">
