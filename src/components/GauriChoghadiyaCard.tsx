...
export const GauriChoghadiyaCard: React.FC<GauriChoghadiyaCardProps> = ({ data, lang, theme }) => {
  ...
  return (
    <div id="gauri-choghadiya-card" className="glass-card rounded-[1.5rem] p-6 sm:p-8">
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    {lang !== "en" && item.name && (
                      <div className="text-[11px] text-stone-400 font-sans mt-0.5">{item.name}</div>
                    )}
                  </div>
                ...
                <div className="text-[11px] text-stone-600 mt-2 font-medium">{loc.nature}</div>

                <div className="mt-3 pt-2 border-t border-stone-200/60 flex items-baseline justify-between">
                  <span className="font-mono text-xs font-bold text-stone-900">{item.start}</span>
                  <span className="text-[11px] text-stone-400">→</span>
                  <span className="font-mono text-xs font-bold text-stone-800">{item.end}</span>
                </div>
              </div>
            );
          })
