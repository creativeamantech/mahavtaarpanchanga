...
export const KundaliChart: React.FC<KundaliChartProps> = ({ planets, lang, theme }) => {
  ...
  planets.forEach((p) => {
    ...
    const matchedRasi = RASI_ORDER.find(
      (r) =>
        cleanRasi.includes(r) ||
        r.includes(cleanRasi) ||
        (p.rasiNumber && RASI_ORDER[p.rasiNumber - 1] === r),
    );
        ...
        <div className="flex items-center space-x-2.5">
          <div
            className={`flex h-8 w-8 items-center justify-center rounded-lg border ${
              isNight
                ? "bg-indigo-950/80 text-amber-300 border-amber-900/40"
          ...
          <div>
            <h3
              className={`text-base font-bold font-serif-vedic ${
      ...
      <div className="mt-5 grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
        ...
        <div className="lg:col-span-8 flex justify-center">
          {chartType === "south" ? (
            ...
            <div
              className={`relative w-full max-w-[420px] aspect-square rounded-2xl border-2 shadow-md p-1 grid grid-cols-4 grid-rows-4 gap-1 select-none ${
                isNight ? "border-indigo-900/60 bg-[#12182b]" : "border-stone-800 bg-stone-900"
              }`}
            >
              {/* Center Logo / Inscription */}
              <div
                className={`col-start-2 col-end-4 row-start-2 row-end-4 rounded-xl border flex flex-col items-center justify-center p-3 text-center ${
                  isNight
              ...
              >
                <span
                  className={`font-serif-vedic text-2xl font-bold ${isNight ? "text-amber-500" : "text-amber-400"}`}
                >
                  ॐ
                </span>
                <span
                ...
                <span
                  className={`text-[9px] mt-1 uppercase tracking-wider ${isNight ? "text-amber-500" : "text-amber-300/80"}`}
                >
                  Observational Drik
                </span>
              </div>
                ...
                return (
                  <button
                    ...
                    className={`relative rounded-xl p-1.5 flex flex-col justify-between text-left transition-all border ${
                      isSelected
                        ...
                        : isNight
                          ? "bg-[#1a233a] border-indigo-900/40 hover:border-indigo-600 hover:bg-[#1f2945]"
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[10px]">
                      <span
                        className={`font-serif-vedic font-bold truncate ${isNight ? "text-amber-400" : "text-amber-300"}`}
                      >
                        {locRasi}
                      </span>
                      <span
                        className={`text-[9px] font-mono ${isNight ? "text-slate-400" : "text-stone-400"}`}
                      >
                        {box.rasiNum}
                      </span>
                    </div>
                      ...
                      {residingPlanets.map((p) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {p.isRetrograde && (
                              <span
                                className={`ml-0.5 text-[8px] ${isNight ? "text-amber-400" : "text-amber-300"}`}
                              >
                                (R)
                              </span>
                            )}
            ...
            <div
              className={`relative w-full max-w-[420px] aspect-square rounded-2xl border shadow-md p-2 flex items-center justify-center ${
            ...
            >
              <svg viewBox="0 0 400 400" className="w-full h-full font-sans">
                ...
                <rect
                  ...
                  width="380"
                  height="380"
                  fill={isNight ? "#0e1424" : "#1c1917"}
                  stroke={isNight ? "#312e81" : "#57534e"}
                  strokeWidth="2"
                  rx="12"
                />
                {/* Diagonal lines */}
                ...
                <polygon
                  ...
                  fill={isNight ? "#1e1b4b" : "#292524"}
                  stroke={isNight ? "#4f46e5" : "#a8a29e"}
                  strokeWidth="2"
                />
                ...
                <text
                  ...
                  y="105"
                  textAnchor="middle"
                  fill={isNight ? "#6ee7b7" : "#34d399"}
                  fontSize="11"
                  fontWeight="bold"
                  fontFamily="monospace"
                >
                  {rasiPlanetsMap[RASI_ORDER[0]]
                    ?.map(
                      (p) =>
                        (GRAHA_ABBR[p.id] ? GRAHA_ABBR[p.id][lang] : p.name.slice(0, 2)) +
                        (p.isRetrograde ? "(R)" : ""),
                    )
                    .join(" ") || "—"}
                </text>
                ...
                <text
                  ...
                  y="80"
                  textAnchor="middle"
                  fill={isNight ? "#fca5a5" : "#f87171"}
                  fontSize="10"
                  fontWeight="bold"
                  fontFamily="monospace"
                >
                  {rasiPlanetsMap[RASI_ORDER[1]]
                    ?.map((p) => (GRAHA_ABBR[p.id] ? GRAHA_ABBR[p.id][lang] : p.name.slice(0, 2)))
                    .join(" ") || ""}
                </text>
        ...
        <div className="lg:col-span-4 flex flex-col justify-center space-y-3">
          ...
          >
            ...
            >
              <div>
                <span
                  className={`text-[10px] uppercase font-bold tracking-wider font-sans block ${
            ...
            <div className="mt-3">
              <span
                className={`text-xs font-semibold block mb-1.5 font-sans ${
              ...
              {selectedPlanets.length === 0 ? (
                <div
                  className={`rounded-xl p-4 text-center text-xs border border-dashed ${
                    isNight
                      ? "bg-[#0e1424] text-slate-500 border-indigo-900/40"
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-xl border p-2.5 flex items-center justify-between shadow-2xs ${
                          isNight
                            ? "border-indigo-900/40 bg-[#0e1424]"
                      ...
                      >
                        <div>
                          <div className="flex items-center space-x-1.5">
                            <span
                              className={`font-bold font-serif-vedic text-sm ${isNight ? "text-slate-200" : "text-stone-900"}`}
                            >
                          ...
                          <span
                            className={`text-[11px] block font-sans ${isNight ? "text-slate-500" : "text-stone-500"}`}
                          >
                            {p.nakshatra} (Pada {p.pada})
                          </span>
                        </div>
                        <span
                          className={`font-mono font-bold text-xs px-2 py-1 rounded-lg border ${
                            isNight
          ...
          {/* Legend */}
          <div
            className={`flex items-center justify-between text-[11px] px-1 font-sans ${isNight ? "text-slate-400" : "text-stone-500"}`}
          >
