...
import { Star, Compass, ArrowRight, ShieldCheck, AlertTriangle, Info, HeartHandshake, Zap, Clock } from "lucide-react";
import { NAKSHATRA_NAMES, calculateNavtara, generateNavtaraTable, type NavtaraResult } from "../lib/navtaraEngine";
import { generateComprehensiveVedicAnalysis, VIMSHOTTARI_SEQUENCE } from "../lib/vedicAstrologyEngine";
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className={`p-6 rounded-2xl border backdrop-blur-xl shadow-lg relative overflow-hidden ${
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div>
            <h2 className={`text-xl font-bold font-devanagari mb-2 flex items-center gap-2 ${
              theme === "nightSky" ? "text-slate-100" : "text-stone-800"
            }`}>
              <Star className="w-5 h-5 text-amber-500" />
              Janma Nakshatra (Birth Star)
            </h2>
            <p className={`text-sm max-w-md ${theme === "nightSky" ? "text-slate-400" : "text-stone-500"}`}>
              Select your birth Nakshatra to generate the complete Navtara Chakra, Tara Dasa sequence, and check for transit Vedha blocks.
            </p>
          </div>
      ...
      {vedicAnalysis.natal_nakshatra_info.is_gandanta && (
        <div className={`p-5 rounded-xl border flex items-start gap-4 ${
          theme === "nightSky" ? "bg-rose-950/20 border-rose-900/50" : "bg-rose-50 border-rose-200"
        }`}>
          <AlertTriangle className="w-6 h-6 text-rose-500 shrink-0 mt-0.5" />
          <div>
            <h3 className={`font-bold mb-1 ${theme === "nightSky" ? "text-rose-400" : "text-rose-700"}`}>
              {vedicAnalysis.natal_nakshatra_info.gandanta_type} Detected
            </h3>
            <p className={`text-sm mb-3 ${theme === "nightSky" ? "text-rose-200/70" : "text-rose-800/70"}`}>
              Your estimated birth degree falls near a critical junction (Gandanta). Special parihara is traditionally advised.
            </p>
            {vedicAnalysis.remedial_measures.gandanta_parihara && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                <div className={`p-3 rounded-lg border ${theme === "nightSky" ? "bg-black/40 border-rose-900/30" : "bg-white/60 border-rose-100"}`}>
              ...
              {vedicAnalysis.transit_vedha_and_override?.is_vedha_blocked && (
                <div className={`mt-4 p-3 rounded-lg border text-left ${
                  ...
                  <p className={`text-xs ${theme === "nightSky" ? "text-slate-300" : "text-stone-600"}`}>
                    High Ashtakavarga score ({vedicAnalysis.transit_vedha_and_override.ashtakavarga_rekhas} Rekhas) neutralizes the malefic transit of {vedicAnalysis.transit_vedha_and_override.base_tara} Tara.
                  </p>
                  <div className="mt-2 text-[10px] uppercase font-bold tracking-wider">
                    <span className={`px-2 py-1 rounded-md ${theme === "nightSky" ? "bg-black/40 text-slate-300" : "bg-white text-stone-600"}`}>
                      Net Malefic Intensity: {vedicAnalysis.transit_vedha_and_override.net_malefic_intensity_percentage}%
                    </span>
                  </div>
                </div>
              )}

              {currentTara.parihara && !vedicAnalysis.transit_vedha_and_override?.is_vedha_blocked && (
                <div className={`mt-4 p-3 rounded-lg border text-left ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map(paryaya => (
            <div key={paryaya} className={`rounded-xl border overflow-hidden ${
              ...
              <div className={`px-4 py-3 border-b text-xs font-bold uppercase tracking-widest flex justify-between items-center ${
                theme === "nightSky" 
                  ? "bg-indigo-950/40 border-indigo-900/40 text-indigo-300" 
                  : "bg-stone-50 border-stone-200 text-stone-600"
              }`}>
                <span>Paryaya {paryaya} {paryaya === 1 ? "(Janma)" : paryaya === 2 ? "(Anugatika)" : "(Mitra)"}</span>
                <span className="opacity-70">Intensity: {paryaya === 1 ? "100%" : paryaya === 2 ? "50%" : "25-75%"}</span>
              </div>
      ...
      <div className={`p-6 rounded-2xl border mt-8 ${
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-6">
          <div>
            ...
            <p className={`text-sm max-w-xl ${theme === "nightSky" ? "text-slate-400" : "text-stone-500"}`}>
              In Tara Dasa, the Dasa names correspond to the 9 Taras, commencing from the strongest Kendra planet in the chart.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-9 gap-3">
          {vedicAnalysis.tara_dasa_analysis.dasa_sequence.map((dasa, idx) => (
            <div key={idx} className={`p-3 rounded-xl border flex flex-col items-center justify-center text-center ${
              theme === "nightSky" 
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                theme === "nightSky" ? "bg-indigo-900/40 text-indigo-300" : "bg-amber-100 text-amber-800"
              }`}>
                {dasa.years} Yrs
              </span>
            </div>
