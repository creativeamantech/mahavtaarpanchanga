...
export const PanchangaSummaryCard: React.FC<PanchangaSummaryCardProps> = ({
  data,
  lang,
  theme,
}) => {
  ...
  const localizedPaksha =
    ...
    (data.paksha || "").toLowerCase().includes("krishna")
      ? t.krishna
      : t.sukla;

  const primaryTithiNum = data.tithi?.[0]?.number || 1;
  const swara = computeSwaraYoga(
    primaryTithiNum,
    data.sunrise,
    data.sunset,
    undefined,
  ...
  return (
    <div id="panchanga-summary-card" className="space-y-6">
      ...
      >
        ...
        <div
          className={`flex flex-wrap items-center justify-between gap-3 border-b pb-6 relative z-10 ${
        ...
        <div className="grid grid-cols-2 gap-3.5 pt-4 sm:grid-cols-4">
          ...
          >
            <span
              className={`text-xs block font-medium ${isNight ? "text-slate-400" : "text-stone-500"}`}
            >
              {t.ayana}
            </span>
            <span
          ...
          >
            ...
            {lang !== "en" && data.sun_rasi && (
              <span
                className={`text-[11px] block capitalize ${isNight ? "text-slate-400" : "text-stone-400"}`}
              >
                {data.sun_rasi}
              </span>
            )}
      ...
      >
        ...
        >
          ...
          <div
            className={`mt-3 border-t pt-2 flex items-center justify-between text-xs ${
              isNight ? "border-slate-800 text-slate-400" : "border-stone-100 text-stone-600"
            }`}
          >
            <span className="font-medium">{t.dayLength}</span>
            <span
              className={`font-bold font-mono ${isNight ? "text-amber-300" : "text-amber-900"}`}
            >
              {data.day_duration}
            </span>
          </div>
          ...
          <div
            className={`mt-2.5 border-t pt-2 space-y-1.5 text-[11px] ${
              isNight ? "border-slate-800/80" : "border-amber-100/80"
            }`}
          >
            <div className="flex items-center justify-between">
              <span className={`font-medium ${isNight ? "text-slate-400" : "text-stone-500"}`}>
                Sunrise Swara:
              </span>
              <div className="flex items-center space-x-1.5">
                ...
                <span
                  className={`inline-flex items-center space-x-1 rounded px-1.5 py-0.5 font-bold ${
                    swara.sunriseSwara === "ida"
                      ? isNight
                        ? "bg-sky-950 text-sky-200 border border-sky-800/50"
                        : "bg-sky-100 text-sky-900"
                      : isNight
                        ? "bg-orange-950 text-orange-200 border border-orange-800/50"
                ...
                {swara.sunriseWindow && (
                  <span
                    className={`text-[10px] font-mono hidden sm:inline ${isNight ? "text-slate-400" : "text-stone-500"}`}
                    title="1 hr from sunrise"
                  >
        ...
        >
          ...
          <div className="mt-3 space-y-2">
            <div className="flex items-baseline justify-between">
              ...
              <span
                id="moonrise-val"
                className={`text-base font-bold font-mono ${isNight ? "text-indigo-200" : "text-stone-900"}`}
              >
                {data.moonrise || "—"}
              </span>
            </div>
          ...
          >
            ...
            <span
              className={`font-bold font-mono ${isNight ? "text-indigo-300" : "text-indigo-900"}`}
            >
              {data.night_duration || "—"}
            </span>
          </div>
          ...
          <div className="mt-3 space-y-2.5">
            <div>
              <div className="flex items-baseline justify-between text-xs">
                <span
                  className={`font-bold font-devanagari ${isNight ? "text-rose-400" : "text-rose-700"}`}
                >
                  {t.rahuKala}
                </span>
                <span
            ...
            {data.abhijit_muhurta && (
              <div>
                <div className="flex items-baseline justify-between text-xs">
                  <span
                    className={`font-bold font-devanagari ${isNight ? "text-emerald-400" : "text-emerald-700"}`}
                  >
                    {t.abhijit}
                  </span>
                  <span
            ...
            {data.brahma_muhurta && (
              <div className="flex items-baseline justify-between text-xs">
                <span
                  className={`font-medium font-devanagari ${isNight ? "text-slate-300" : "text-stone-700"}`}
                >
                  {t.brahmaMuhurta}
                </span>
        ...
        >
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
            ...
            >
              <span
                className={`block text-[11px] font-medium ${isNight ? "text-slate-400" : "text-stone-500"}`}
              >
                {t.sakaEra}
              </span>
              <span
                className={`font-extrabold font-mono text-sm ${isNight ? "text-slate-100" : "text-stone-900"}`}
              >
                {data.saka_year}
              </span>
            </div>
            ...
            >
              ...
              <span
                className={`font-mono font-extrabold text-xs truncate block ${isNight ? "text-slate-100" : "text-stone-900"}`}
              >
                {data.kali_day}
              </span>
            </div>
          ...
          <div
            className={`mt-3 border-t pt-2 text-[11px] font-mono truncate ${
              isNight ? "border-slate-800 text-slate-400" : "border-stone-100 text-stone-500"
            }`}
          >
            {data.ayanamsa || "Tropical"} (
            {data.ayanamsa_degrees != null ? `${data.ayanamsa_degrees.toFixed(4)}°` : "0°"})
          </div>
        </div>
      </div>
    </div>
