...
export const PlanetaryPositionsCard: React.FC<PlanetaryPositionsCardProps> = ({
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className={`flex flex-col sm:flex-row sm:items-center sm:justify-between border-b pb-3.5 gap-3 ${
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
          ...
          >
            {t.ayanamsaSystem}:{" "}
            {data.ayanamsa_degrees != null ? `${data.ayanamsa_degrees.toFixed(4)}°` : "0.0000°"} (
      ...
      {displayMode === "kundali" ? (
        <KundaliChart
          planets={planets}
          lang={lang}
          coordinateMode={data.coordinate_label}
          theme={theme}
        />
      ) : displayMode === "transitions" ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[740px] text-left text-sm">
            <thead>
              <tr
                className={`border-b text-xs font-bold uppercase tracking-wider font-sans ${
                  isNight
            ...
            <tbody
              className={`divide-y font-sans ${isNight ? "divide-indigo-900/40" : "divide-stone-100"}`}
            >
              {planets.map((planet) => {
                  ...
                  >
                    ...
                    <td className="px-3.5 py-3.5">
                      <div className="flex items-center space-x-2">
                        <span
                          className={`flex h-7 w-7 items-center justify-center rounded-lg font-bold text-sm font-devanagari ${
                            isNight
                        ...
                        <div>
                          <div
                            className={`font-bold font-devanagari flex items-center gap-1.5 ${
                          ...
                          >
                            ...
                            {transitStatus?.isCombust && (
                              ...
                              >
                                <Flame
                                  className={`h-2.5 w-2.5 mr-0.5 ${isNight ? "text-amber-500" : "text-amber-600"}`}
                                />
                                {t.combust || "Asta"}
                              </span>
                            )}
                          ...
                          {lang !== "en" && (
                            <div
                              className={`text-[11px] ${isNight ? "text-slate-400" : "text-stone-400"}`}
                            >
                              {planet.name}
                            </div>
                          )}
                    ...
                    <td className="px-3.5 py-3.5">
                      <div
                        className={`font-semibold font-devanagari ${isNight ? "text-slate-200" : "text-stone-900"}`}
                      >
                        {rasiLabel}
                      </div>
                      <div
                    ...
                    {/* Degrees in Sign */}
                    <td
                      className={`px-3.5 py-3.5 font-mono text-xs font-semibold ${isNight ? "text-slate-300" : "text-stone-800"}`}
                    >
                      <div>{planet.degreesInRasi}</div>
                      {transitStatus && (
                        <div
                          className={`text-[10px] font-sans ${isNight ? "text-slate-500" : "text-stone-400"}`}
                        >
                    ...
                    <td className="px-3.5 py-3.5">
                      <div
                        className={`font-medium font-devanagari ${isNight ? "text-slate-200" : "text-stone-900"}`}
                      >
                        {nakLabel}
                      </div>
                      <div
                    ...
                    <td className="px-3.5 py-3.5 text-center">
                      {planet.isRetrograde ? (
                        ...
                        >
                          <ArrowUpRight
                            className={`mr-1 h-3 w-3 ${isNight ? "text-slate-500" : "text-stone-400"}`}
                          />
                          {t.direct}
                        </span>
                      )}
