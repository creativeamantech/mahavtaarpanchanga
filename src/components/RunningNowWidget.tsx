...
export const RunningNowWidget: React.FC<RunningNowWidgetProps> = ({ data, lang, theme }) => {
  ...
  if (inWindow(data.gulika_kala)) {
    activeMuhurtas.push({
      name: lang === "hi" ? "गुलिका काल" : "Gulika Kala",
      type: "inauspicious",
    });
  }
  if (inWindow(data.abhijit_muhurta)) {
    activeMuhurtas.push({
      name: lang === "hi" ? "अभिजित मुहूर्त" : "Abhijit Muhurta",
      type: "auspicious",
    });
  }
  if (inWindow(data.brahma_muhurta)) {
    activeMuhurtas.push({
      name: lang === "hi" ? "ब्रह्म मुहूर्त" : "Brahma Muhurta",
      type: "auspicious",
    });
  }
  if (data.amrita_kala) {
    for (const am of data.amrita_kala) {
      if (inWindow(am)) {
        activeMuhurtas.push({
          name: lang === "hi" ? "अमृत काल" : "Amrita Kala",
          type: "auspicious",
        });
      }
    }
  ...
  // Format current time
  const timeDisplay = now.toLocaleTimeString(lang === "hi" ? "hi-IN" : "en-US", {
    hour: "numeric",
    minute: "2-digit",
    second: "2-digit",
  });
  ...
  return (
    <div
      className={`rounded-2xl p-5 shadow-md flex flex-col gap-4 transition-all ${
        isNight
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          <h3
            className={`text-xs font-bold uppercase tracking-wider ${isNight ? "text-indigo-300" : "text-emerald-700"}`}
          >
            {lang === "hi" ? "पंचांग व मुहूर्त" : "Panchanga & Muhurta"}
          </h3>

          <div
            className={`rounded-xl p-3 space-y-2 border ${isNight ? "bg-slate-900/50 border-slate-700" : "bg-white/60 border-emerald-100"}`}
          >
          ...
          <div
            className={`text-xs p-2 rounded-lg border ${isNight ? "bg-indigo-900/30 text-indigo-300 border-indigo-800" : "bg-amber-50 text-amber-800 border-amber-200"}`}
          >
            <div className="flex items-center gap-1.5 font-medium">
              <Sun className="w-3.5 h-3.5" />
              {lang === "hi"
                ? `सूर्योदय के समय तिथि: ${sunriseTithiName}`
                : `Tithi at Sunrise: ${sunriseTithiName}`}
            </div>
          </div>
        ...
        <div className="space-y-3">
          ...
          <div
            className={`rounded-xl p-3 space-y-3 border ${isNight ? "bg-slate-900/50 border-slate-700" : "bg-white/60 border-emerald-100"}`}
          >
            {activeHora ? (
              <div className="flex items-center justify-between text-sm">
                <span className="text-stone-500 dark:text-slate-400">
                  {lang === "hi" ? "सक्रिय होरा:" : "Active Hora:"}
                </span>
                <span
                  className={`font-bold font-devanagari ${isNight ? "text-rose-300" : "text-rose-600"}`}
                >
                  {lang === "hi"
                    ? horaNamesHi[activeHora.ruler] || activeHora.ruler
                    : activeHora.ruler}{" "}
                  (
                  {activeHora.isDay
                    ? lang === "hi"
            ...
            <div className="flex items-center justify-between text-sm">
              <span className="text-stone-500 dark:text-slate-400">
                {lang === "hi" ? "प्रवहमान नाड़ी:" : "Flowing Nadi:"}
              </span>
              <span
            ...
            {activeTattva ? (
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-bold font-devanagari">
                    {lang === "hi"
                      ? tattvaNamesHi[activeTattva.sanskrit] || activeTattva.sanskrit
                      : activeTattva.sanskrit}
                  </span>
                </div>
              </div>
            ) : null}
            ...
            {swaraData.activeCelestialWindow && (
              <div
                className={`mt-2 rounded-lg p-2.5 text-xs flex flex-col gap-1.5 ${
                  isNight
                    ? "bg-indigo-900/30 text-indigo-200 border border-indigo-800/50"
              ...
              >
                <div className="font-bold flex items-center gap-1.5 text-sm">
                  ...
                  {swaraData.activeCelestialWindow === "sunrise"
                    ? lang === "hi"
                      ? `सूर्योदय स्वर काल ${swaraData.sunriseSwara === "ida" ? "(इड़ा)" : "(पिङ्गला)"}`
                      : `Sunrise Swara Window ${swaraData.sunriseSwara === "ida" ? "(Ida)" : "(Pingala)"}`
                    : ""}
                  {swaraData.activeCelestialWindow === "midday"
                    ? lang === "hi"
                      ? `मध्याह्न स्वर काल ${swaraData.sunriseSwara === "ida" ? "(इड़ा)" : "(पिङ्गला)"}`
                      : `Midday Swara Window ${swaraData.sunriseSwara === "ida" ? "(Ida)" : "(Pingala)"}`
                    : ""}
                  {swaraData.activeCelestialWindow === "sunset"
                    ? lang === "hi"
                      ? `सूर्यास्त स्वर काल ${swaraData.sunsetSwara === "ida" ? "(इड़ा)" : "(पिङ्गला)"}`
                      : `Sunset Swara Window ${swaraData.sunsetSwara === "ida" ? "(Ida)" : "(Pingala)"}`
                    : ""}
                  {swaraData.activeCelestialWindow === "moonrise"
                    ? lang === "hi"
                      ? `चन्द्रोदय स्वर काल ${swaraData.moonriseSwara === "ida" ? "(इड़ा)" : "(पिङ्गला)"}`
                      : `Moonrise Swara Window ${swaraData.moonriseSwara === "ida" ? "(Ida)" : "(Pingala)"}`
                    : ""}
                  {swaraData.activeCelestialWindow === "moonset"
                    ? lang === "hi"
                      ? `चन्द्रास्त स्वर काल ${swaraData.moonsetSwara === "ida" ? "(इड़ा)" : "(पिङ्गला)"}`
                      : `Moonset Swara Window ${swaraData.moonsetSwara === "ida" ? "(Ida)" : "(Pingala)"}`
                    : ""}
                </div>
            ...
            {activeTithiSwaraEvent && (
              ...
              >
                <div className="font-bold flex items-center gap-1.5 text-sm">
                  <Wind className="w-3.5 h-3.5" />
                  {lang === "hi"
                    ? `तिथि ${activeTithiSwaraEvent.type === "start" ? "आरंभ" : "समापन"} स्वर काल`
                    : `Tithi ${activeTithiSwaraEvent.type === "start" ? "Start" : "End"} Swara`}
            ...
            {activeNakshatraSwaraEvent && (
              <div
                className={`mt-2 rounded-lg p-2.5 text-xs flex flex-col gap-1.5 ${
                  isNight
                    ? "bg-fuchsia-900/30 text-fuchsia-200 border border-fuchsia-800/50"
              ...
              >
                <div className="font-bold flex items-center gap-1.5 text-sm">
                  <Wind className="w-3.5 h-3.5" />
                  {lang === "hi"
                    ? `नक्षत्र ${activeNakshatraSwaraEvent.type === "start" ? "आरंभ" : "समापन"} स्वर काल`
                    : `Nakshatra ${activeNakshatraSwaraEvent.type === "start" ? "Start" : "End"} Swara`}
