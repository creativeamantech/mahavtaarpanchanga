...
export function VedicHorasView({ panchangaData }: { panchangaData: PanchangaResponse }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
        ...
        {activeHora && activeTattva ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="rounded-2xl bg-linear-to-br from-indigo-50 to-purple-50 p-6 border border-indigo-100 shadow-inner flex flex-col items-center justify-center space-y-4 text-center">
              <div className="text-sm font-semibold text-indigo-800 uppercase tracking-widest">
                Active Planetary Ruler
              </div>
              ...
              <div className="flex space-x-3 mt-2">
                <span
                  className={`px-4 py-1.5 rounded-full text-sm font-bold border ${activeHora.nadi === "ida" ? "bg-sky-100 text-sky-800 border-sky-300" : activeHora.nadi === "pingala" ? "bg-orange-100 text-orange-800 border-orange-300" : "bg-purple-100 text-purple-800 border-purple-300"}`}
                >
                  Nadi:{" "}
                  {activeHora.nadi === "ida"
                    ? "Ida 🔵"
            ...
            <div className="rounded-2xl bg-white p-6 border border-stone-100 shadow-xs flex flex-col justify-center space-y-4">
              <div>
                <div className="flex justify-between text-xs font-bold text-stone-500 mb-1 uppercase tracking-wider">
                  ...
                  <span>
                    {Math.round((now - activeHora.startTimeMs) / 60000)} /{" "}
                    {Math.round(activeHora.durationMs / 60000)} mins
                  </span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-xs font-bold text-stone-500 mb-1 uppercase tracking-wider">
                  ...
                  <span>
                    {Math.round((now - activeTattva.startTimeMs) / 60000)} /{" "}
                    {Math.round(activeTattva.durationMs / 60000)} mins
                  </span>
                </div>
                <div className="h-3 w-full bg-stone-100 rounded-full overflow-hidden relative border border-stone-200">
                  <div
      ...
      <div className="glass-card rounded-[2rem] p-6 shadow-sm transition-all">
        <h2 className="mb-4 text-lg font-bold text-stone-800">Daily Schedule: 24 Vedic Horas</h2>
